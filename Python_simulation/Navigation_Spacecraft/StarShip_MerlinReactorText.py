...
def create_command_module():
    capsule = trimesh.creation.cone(radius=1.0, height=2.0, sections=64)
    capsule.apply_translation([0, 0, FUSELAGE_LENGTH + 1.0])
    return capsule

def create_engines():
    engines = []
    for offset in [-0.7, 0.7]:
        base = trimesh.creation.cylinder(radius=0.3, height=1.2)
        base.apply_translation([offset, 0, 0.5])
        nozzle = trimesh.creation.cone(radius=0.25, height=0.6)
        nozzle.apply_translation([offset, 0, 0])
        engines.extend([base, nozzle])
    return engines
...
def create_shield_alternative():
    # Toro simulado con múltiples tubos circulares
    shield = []
...
def plot_mesh(mesh):
    ...
    collection.set_facecolor((0.25, 0.4, 0.65, 0.95))  # Azul oscuro
    collection.set_edgecolor((0.05, 0.05, 0.05, 0.3))  # Borde gris tenue
    ...
    plt.tight_layout()
    plt.savefig("Starship_FalconStyle.png", dpi=300)
    plt.show()
...
def main():
    ...
    parts.append(create_command_module())
    parts.extend(create_engines())
